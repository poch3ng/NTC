以下提供一個「包含 ISA、GS、ST 外框」+「在 ST 區段內解析 N9、N1、PO1（含巢狀 N1 Sub-Loop）、CTT**」的完整 VB.NET 示範程式。此程式示範了最常見的 EDI 結構：

ISA / IEA：Interchange 開頭、結尾

GS / GE：Functional Group 開頭、結尾

ST / SE：Transaction Set 開頭、結尾
...
PO1 Loop（含 PO1 內的 N1 Sub-Loop）

CTT Loop



> 注意：真實 EDI 可能包含多個 GS、多個 ST。本範例示意「一次只處理一個 GS 及其底下的多個 ST」，或「只示範一個 ST」。若要支援「一個檔內多個 GS、多個 ST」的情況，做法類似：每當遇到新的 GS/ST，就關閉前一個後再開新的，各自收集並處理。此處著重在「ST 內的各種 Loop 處理」的主邏輯。
