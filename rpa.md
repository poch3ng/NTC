...
假設起始值為 2，間隔為 5，第 n 個 idx 的公式為：

idx = 2 + (n - 1) * 5

n 是當前變數的位置（從 1 開始）。

第 1 個變數對應 idx = 2，第 2 個變數對應 idx = 7，以此類推。
...
在每次迴圈中，使用 Assign 活動計算 idx：

idx = 2 + (n - 1) * 5

Step 3: 動態設置 Selector

在 Type Into 活動中，設置動態 Selector：

<webctrl tag='INPUT' idx='{{idx}}' />

Step 4: 迴圈處理

1. 使用 For Each 遍歷變數清單。
...
1. 有一個變數清單：{"Variable1", "Variable2", "Variable3"}。


2. 需要將它們分別輸入到 idx=2, 7, 12 的欄位。
...
可以使用 UI Explorer 驗證 Selector。



2. 例外處理

如果輸入框動態加載，使用 On Element Appear 或 Delay 確保元素已準備好。



3. 測試與調整

先針對小範圍（如 2-3 個變數）測試，確保動態 Selector 可用。
